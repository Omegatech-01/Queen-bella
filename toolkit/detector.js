    ...
    async function keyChecker(url,key){
      try {
        ...
        if(Data.notify.h == 1 && Exp.authState){
          ...
          let inf= '\n\n> _If this feels annoying, you can disable it by typing .set keyChecker off*_'
            ...
            if(usage >= limit && (!Data.notify?.reset||interval)) {
              ...
              for(let i of kfeatures){
                ...
                if(use >= max && (!Data.notify?.[i]||Date.now() >= Data.notify[i])){
                  ...
                  let msg = i.includes('elevenlabs') ? '> _The text-to-speech feature .elevenlabs cannot be used until the limit is reset!_'
                    :i.includes('filters') ? '> _Features like .filters such as .toanime and .toreal cannot be used until the limit is reset!_'
                    :i.includes('luma') ? '> _The .i2v or .img2video feature cannot be used until the limit is reset!_'
                    :i.includes('logic-bell') ? '_Auto AI chat will not respond/cannot be used until the limit is reset!_'
                    :i.includes('enlarger') ? '_The .enlarger feature cannot be used until the limit is reset!_'
                    :''
      ...
      for(let [id,b] of chatDb){
        ...
        let w = '5 menit'
          ...
          let text = (data.now == 'magrib' && ramadhan)
            ? `*Hello all Muslims in the group ${subject}!

Happy Iftar 🍽️! May your fast be accepted by Allah SWT.
The prayer time ${data.now}${emoji} in the area ${b.jadwalsholat.v} has begun! Don’t forget to perform your prayers on time.

"Allahumma laka shumtu wa bika aamantu wa ‘ala rizq-ika aftartu, bi rahmatika ya arhamar rahimin."
(O Allah, to You I fast, to You I believe, and with Your provision, I break my fast. With Your mercy, O Most Merciful of the merciful).
