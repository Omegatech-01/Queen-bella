...
export default async function on({ cht, Exp, store, ev, is }) {
            ...
            for(let i of data){
               ...
               cards.push({
                 ...
                 nativeFlowMessage: {
                     ...
                     {
                       name: "cta_url",
                       buttonParamsJson: '{"display_text":"WhatsappChannel","url":"https://whatsapp.com/channel/0029VawHbTr1CYoWf3uLwz3r","webview_presentation":null}',
                     },
                   ],
                 },
               })
	...
	ev.on({
	  ...
	  try {
		...
		let s = await exif["writeExifImg"](buff, {
			packname: 'Queen-bella|Powered by ☆OmegaTech',
			author: 'ⒸQueen-bella' + cht.pushName
		}, true)
		Exp.sendMessage(id, {
			sticker: {
				url: s
	...
	ev.on({ 
        ...
        tag: 'search',
        args: `Example: ${cht.msg} Xun'er`,
        badword: true,
        energy: 5
    }, async() => {
        let url = await await fetch(api.xterm.url + "/api/search/google-image?query="+cht.q).then(async a => (await a.json()).data.getRandom())
        Exp.sendMessage(id, { image: { url, }, caption: `Google image search|POWERED BY OMEGATECH: \`${cht.q}\`` }, { quoted: cht })
	...
	ev.on({ 
        ...
        if(data.length < 1) return cht.reply("Video not found!")
    ...
    ev.on({
        ...
        media: {
			type: ["image", "sticker"],
			msg: `Reply gambar/sticker with caption ${cht.prefix}animesearch to search anime!`,
			save: false
		},
		energy: 5
    }, async ({ media }) => {
        try {
          ...
          const apiUrl = `https://api.trace.moe/search?url=${encodeURIComponent(link)}`
            ...
            if (!res.ok || !result || result.length < 1) {
                return cht.reply("❌ No anime information found for the provided image.")
            }
            ...
            console.error(error)
            cht.reply("❌ An error occurred while contacting the API. Please try again later.")
        }
    })
