...
export default async function on({ cht, Exp, store, ev, is }) {
    ...
    ev.on({ 
        ...
        //    txt += "\n🪙Coins : " + user.coins //(coming soon)
            ...
            if(user.premium.time >= Date.now()){
              ...
              txt += `\n🗓️Expired on: ${func.dateFormatter(user.premium.time, "Africa/Freetown")}`
            } else {
            ...
            txt += "\n- Last Charge: " + func.dateFormatter(user.lastCharge, "Africa/Freetown")
        const menu = {
            text: txt,
            contextInfo: { 
                externalAdReply: {
    ...
    ev.on({ 
        ...
        let finish = func.dateFormatter(est, "Africa/Freetown")
        if(!user.charging){
            if(user.energy < max){
                user.charging = true
        ...
        const mess = {
            ...
            contextInfo: { 
                externalAdReply: {
                    ...
                    sourceUrl: "https://github.com/Omegatech-01",
                    mediaUrl: `http://ẉa.me/23234675912/${user.charging ? "2733":"2734"}`,
                    renderLargerThumbnail: true,
                    showAdAttribution: true,
                    mediaType: 1,
                }
...
_Code ${code} akan dihapus dalam kurun waktu ${time}_
  `;
    ...
    ev.on({ 
      ...
      let _code = code.code
      let sentAt = func.dateFormatter(code.exp - parseTimeString(time), "Africa/Freetown")
      if(!code) {
...
- Last Online: ${func.dateFormatter(cht.memories.lastChat, "Africa/Freetown")}`, { mentions: [Sender.split("@")[0]+from.sender] })
    })
