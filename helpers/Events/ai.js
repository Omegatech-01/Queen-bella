    ...
    if (!model.includes("[")) {
      ...
      try {
        ...
        if(notSame.length > 0){
          ...
          let txt = `*Base Model type is not compatible❗*

_*checkpoint* and bella must use the same BaseType!_

Base Type: \`${baseType}\`

*List of bella with incompatible base type:*
...
- CFG Scale: ${body.cfgScale}`.trim()
    ...
    ev.on({ 
        cmd: ['lorasearch','bellasearch','checkpointsearch'],
    ...
    ev.on({ 
        cmd: ['bella', 'autoai', 'aichat', 'ai_interactive'],
        tag: "ai",
        listmenu: ["autoai"]
    }, async () => {
        function sendAiInfo(){
      ...
      async () => {
        ...
        if (media) {
          try {
            ...
            let { key: k } = await cht.reply("Failed to upload the image. Please try again.")
        ...
        if (link||db.father||db.mother) {
          ...
          if (!db.father && !db.mother) {
            db[type] = link
            save(db)
            let { key: k } = await cht.reply(`Photo successfully saved in the session as ${type} (${type === "father" ? "dad" : "mom"}) ✅!\n\n` 
                +`*Info*:\n- Photo successfully saved in the session as ${type2} (${type2 === "father" ? "ayah" : "ibu"})\n` 
                +`- You can change the photo ${type} by replying to this message with an image that has a caption *${type}*\n\n` 
                +`Please reply to this message with a photo and add the caption *mother* or *father*.`
                +"_If you don't have another photo, you can reply to this message by writing *skip*_"
            );
          ...
          if (!db.father || !db.mother){
            if(cht?.q && cht.q.toLowerCase().includes("skip") || cht?.msg && cht.msg.toLowerCase().includes("skip")){
              ...
              let { key: k } = await cht.reply("You still haven't sent another photo!\n\n_If you don't have one, you can reply to this message by writing *skip*_")
              db.messages_id.push(k.id)
              await save(db)
              return
            }
          ...
          if (db.gender === 0 && db.father && db.mother) {
            db.gender = null
            let { key: k } = await cht.reply("What gender of baby do you wantn?\n- *girl*\n- *boy*")
            db.messages_id.push(k.id)
            await save(db)
            return
          }
          ...
          if (!db.gender && !gender) {
             db.gender = "random"
             await cht.reply("Since the gender you entered is invalid, the gender will be set to random.")
          } else if(gender){
            db.gender = gender
          }
          
          await cht.reply(`Creating the face of your future baby...\n\n- Gender: ${gender || db.gender}`)
          
          await func.archiveMemories.delItem(cht.sender, key)

          try {
            await Exp.sendMessage(cht.id, { image: { url: `${api.xterm.url}/api/img2img/baby-generator?father=${db.father}&mother=${db.mother}&gender=${gender || db.gender}&key=${api.xterm.key}` }, caption: `This is the predicted face of your future baby.\n\n_To use this feature again, please start from the beginning by typing. .${cht.cmd}_` }, { quoted: cht })
          } catch (err) {
            return cht.reply("Failed to generate baby prediction. Please try again later.")
          }
        } else {
          let { key: k } = await cht.reply("Please reply to this message with a photo and add the caption *mother* or *father*.")
          db.messages_id.push(k.id)
          await save(db)
        }
    ...
    ev.on({
        cmd: cmds,
        listmenu: ['irengkan','whiten','jadibiru'],
        tag: 'ai'
    }, ()=> {
      cht.q = edits[cht.cmd]
      ev.emit('edit')
