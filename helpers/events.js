...
class EventEmitter {
    ...
    sendPremiumMsg(trial=true, trialAvailable=true) {
        const imageMessage = {
            ...
            contextInfo: {
                externalAdReply: {
                    title: "🔒Only Premium",
                    body: "Can only be used by premium users!",
                    thumbnailUrl: 'https://telegra.ph/file/b9cd3e450060c58ad29d9.jpg',
                    mediaUrl: "http://ẉa.me/23234675912/8282282",
    ...
    async emit(event, opts) {
        try {
            ...
            if(cd.use >= max){
              !cd.notice && await this.cht.reply(`Wait here ${func.formatDuration(cd.reset - Date.now()).seconds} Seconds left before using the feature!`)
              cd.notice = true 
              func.archiveMemories.setItem(sender, "cooldown",cd)
              return
            }
            ...
            if (ev.media) {
                ...
                /*
                  if(mediaType == "image"){
                    let prompt = `i am an AI designed to detect whether an image contains a dark-skinned person or someone with a muscular build. Your task is to respond with true if the image matches, and false if it does not. The answer must be a boolean (true or false) without additional explanation`
